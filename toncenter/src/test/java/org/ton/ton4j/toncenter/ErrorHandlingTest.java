...
public class ErrorHandlingTest {
  ...
  public void testInvalidApiKeyErrorMessage() {
    TonCenter client = TonCenter.builder().apiKey(INVALID_API_KEY).testnet().build();
    ...
    try {
      ...
      assertTrue(
          ...
          e.getMessage().contains("API key does not exist") || e.getMessage().contains("API key"));
  ...
  public void testMainnetWithInvalidApiKey() {
    ...
    try {
      ...
      assertTrue(
          "Should contain API key error",
          e.getMessage().contains("API key") || e.getMessage().contains("401"));
    } catch (TonCenterException e) {
