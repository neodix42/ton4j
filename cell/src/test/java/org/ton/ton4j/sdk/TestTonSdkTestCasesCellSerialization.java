...
public class TestTonSdkTestCasesCellSerialization {
  public static final String numbersTestFileUrl =
      "https://raw.githubusercontent.com/neodix42/ton-sdk-test-cases/main/cell-serialization.json";
  ...
  public void testCellSerialization1() {
    ...
    boolean bocWithCacheBits =
        Boolean.parseBoolean(testCase.getInput().get("bocWithCacheBits").toString());
    boolean bocWithTopHash =
        Boolean.parseBoolean(testCase.getInput().get("bocWithTopHash").toString());
    boolean bocWithIntHashes =
        Boolean.parseBoolean(testCase.getInput().get("bocWithIntHashes").toString());
    ...
    String actualBocAsHex =
        cell.toHex(bocWithCrcC, bocWithIndex, bocWithCacheBits, bocWithTopHash, bocWithIntHashes);
    ...
    assertThat(actualBocAsHex.toUpperCase()).isEqualTo(expectedBocAsHex);
    assertThat(Utils.bytesToHex(cell.getHash()).toUpperCase()).isEqualTo(expectedCellHash);
  }
  ...
  public void testCellSerialization2() {
    ...
    byte[] cell1_bytes =
        gson.fromJson(testCase.getInput().get("cell1_bytes").toString(), byte[].class);
    BigInteger cell1_uint256 =
        new BigInteger(testCase.getInput().get("cell1_uint256").toString(), 16);
    ...
    String actualBocAsHexWithCrcOnly =
        cell2.toHex(true, false, bocWithCacheBits, bocWithTopHash, bocWithIntHashes);
    String actualBocAsHexWithIndexOnly =
        cell2.toHex(false, true, bocWithCacheBits, bocWithTopHash, bocWithIntHashes);
    String actualBocAsHexWithCrcAndIndex =
        cell2.toHex(true, true, bocWithCacheBits, bocWithTopHash, bocWithIntHashes);

    String expectedCellOutput = testCase.getExpectedOutput().get("cellOutput").toString();
    String expectedBocAsHexWithCrcOnly =
        testCase.getExpectedOutput().get("bocAsHexWithCrcOnly").toString();
    String expectedBocAsHexWithIndexOnly =
        testCase.getExpectedOutput().get("bocAsHexWithIndexOnly").toString();
    String expectedBocAsHexWithCrcAndIndex =
        testCase.getExpectedOutput().get("bocAsHexWithCrcAndIndex").toString();
    String expectedCellHash = testCase.getExpectedOutput().get("cellHash").toString();

    assertThat(StringUtils.trim(cell2.print())).isEqualTo(expectedCellOutput);

    assertThat(actualBocAsHexWithCrcOnly.toUpperCase()).isEqualTo(expectedBocAsHexWithCrcOnly);
    assertThat(actualBocAsHexWithIndexOnly.toUpperCase()).isEqualTo(expectedBocAsHexWithIndexOnly);
    assertThat(actualBocAsHexWithCrcAndIndex.toUpperCase())
        .isEqualTo(expectedBocAsHexWithCrcAndIndex);
  ...
  public void testCellSerialization3() {
    ...
    String expectedBocAsHexWithCrcOnly =
        testCase.getExpectedOutput().get("cell5bocAsHexWithCrcOnly").toString();
    String cell5Hash = testCase.getExpectedOutput().get("cell5Hash").toString();
    Integer cell5Depth =
        Integer.parseInt(testCase.getExpectedOutput().get("cell5Depth").toString());
    String expectedCell5RefsDescriptorHex =
        testCase.getExpectedOutput().get("cell5RefsDescriptorHex").toString();
    String expectedCell5BitsDescriptorHex =
        testCase.getExpectedOutput().get("cell5BitsDescriptorHex").toString();

    assertThat(StringUtils.trim(cell5.print())).isEqualTo(expectedCellOutput);

    assertThat(cell5.toHex(true).toUpperCase()).isEqualTo(expectedBocAsHexWithCrcOnly);
    assertThat(Utils.bytesToHex(cell5.getHash()).toUpperCase()).isEqualTo(cell5Hash);

    assertThat(cell5.getDepthLevels()[0]).isEqualTo(cell5Depth);
    assertThat(Utils.bytesToHex(cell5.getRefsDescriptor(0)))
        .isEqualTo(expectedCell5RefsDescriptorHex);
    assertThat(Utils.bytesToHex(cell5.getBitsDescriptor()))
        .isEqualTo(expectedCell5BitsDescriptorHex);
  }
}
