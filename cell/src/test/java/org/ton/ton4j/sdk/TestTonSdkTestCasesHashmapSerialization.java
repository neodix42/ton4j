...
public class TestTonSdkTestCasesHashmapSerialization {
  public static final String numbersTestFileUrl =
      "https://raw.githubusercontent.com/neodix42/ton-sdk-test-cases/main/hashmap-serialization.json";
  ...
  public void testHashmapSerialization1() {
    ...
    String expectedCellBocWithCrc =
        testCase.getExpectedOutput().get("HashMapBocWithCrc").toString();
  ...
  public void testHashmapSerialization6() {
    ...
    String expectedCellBocWithCrc =
        testCase.getExpectedOutput().get("cellWithHashMapBocWithCrc").toString();
  ...
  public void testHashmapSerialization10() {
    ...
    int keySizeBits = Integer.parseInt(testCase.getInput().get("keySizeBits").toString());
    //        int valueSizeBits =
    // Integer.parseInt(testCase.getInput().get("valueSizeBits").toString());
    Long key1 = Long.parseLong(testCase.getInput().get("key1").toString());
    Long value1 = Long.parseLong(testCase.getInput().get("value1").toString());
    Long key2 = Long.parseLong(testCase.getInput().get("key2").toString());
    ...
    Cell cellDict =
        x.serialize(
            k -> CellBuilder.beginCell().storeUint((Long) k, keySizeBits).endCell().getBits(),
            //                v -> CellBuilder.beginCell().storeUint((Long) v,
            // valueSizeBits).endCell()
            v -> CellBuilder.beginCell().storeUint((Long) v, 3).endCell());
    ...
    String expectedCellWithHashMapPrint =
        testCase.getExpectedOutput().get("cellWithHashMapPrint").toString();
    String expectedCellWithHashMapHash =
        testCase.getExpectedOutput().get("cellWithHashMapHash").toString();
    String expectedCellWithHashMapBocWithCrc =
        testCase.getExpectedOutput().get("cellWithHashMapBocWithCrc").toString();
    ...
    // good -
    // B5EE9C7241010701001F000201200102020120030402012005060003BE430003BC850003BAC70003B9099A785C76
    assertThat(cellDict.toHex().toUpperCase())
        .isEqualTo(expectedCellBocWithCrc); // same as pytoniq-core

    Cell cellWithDict = CellBuilder.beginCell().storeDict(cellDict).endCell();

    assertThat(StringUtils.trim(cellWithDict.print())).isEqualTo(expectedCellWithHashMapPrint);
    assertThat(Utils.bytesToHex(cellWithDict.hash()).toUpperCase())
        .isEqualTo(expectedCellWithHashMapHash);
    assertThat(cellWithDict.toHex().toUpperCase())
        .isEqualTo(expectedCellWithHashMapBocWithCrc); // same as pytoniq-core
  }
}
