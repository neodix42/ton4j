...
public class TestHashMap {
    ...
    public void testHashMapDeserialization() {
        ...
        log.info("cell {}", Utils.bytesToHex(cell.toBoc(true)));
        ...
        log.info("cell {}", Utils.bytesToHex(cellSerialized.toBoc(false)));
    ...
    @Test
    @Ignore
    public void testHashMapDeserializationTwoDicts() {
        int keySizeX = 9;
        TonHashMap x = new TonHashMap(keySizeX);
        ...
        int keySizeY = 64;
        TonHashMap y = new TonHashMap(keySizeY);
        ...
        log.info("serialized cellY {}", cellY.print());

        CellSlice cs = CellSlice.beginParse(cellX);
        TonHashMap dex = cs.loadDict(keySizeX,
                k -> k.readUint(keySizeX),
                v -> CellSlice.beginParse(v).loadUint(3)
        );

        log.info("Deserialized hashmap from cellX");
        for (Map.Entry<Object, Object> entry : dex.elements.entrySet()) {
            log.info("key {}, value {}", entry.getKey(), entry.getValue());
        }

        cs = CellSlice.beginParse(cellY);
        TonHashMap dey = cs.loadDict(keySizeY,
                k -> k.readUint(keySizeY),
                v -> CellSlice.beginParse(v).loadUint(8)
        );

        log.info("Deserialized hashmap from cellY");
        for (Map.Entry<Object, Object> entry : dey.elements.entrySet()) {
            log.info("key {}, value {}", entry.getKey(), entry.getValue());
        }
        ...
        TonHashMap loadedDictX = cs.loadDict(keySizeX,
                k -> k.readUint(keySizeX),
                v -> CellSlice.beginParse(v).loadUint(3)
        );
        ...
        TonHashMap loadedDictY = cs.loadDict(keySizeY,
                k -> k.readUint(keySizeY),
                v -> CellSlice.beginParse(v).loadUint(8)
        );
    ...
    public void testHashMapDeserializationWithMixedEmptyEdges() {
        ...
        Cell cell = Cell.fromBoc(Utils.base64ToBytes(BOC_NETWORK_CONFIG));
        log.info("cell:\n{}", cell.print());
