...
public class CellNew {
    ...
    private static final int depthSize = 2;

    public BitString bits;
    public List<CellNew> refs;
    ...
    public CellNew(BitString bits, int bitSize, List<CellNew> refs, boolean special, byte levelMask) {
    ...
    static CellNew fromBoc(String data) {
        return fromBocMultiRoot(Utils.hexToUnsignedBytes(data)).get(0);
    }

    static CellNew fromBoc(int[] data) {
        return fromBocMultiRoot(data).get(0);
    }

    public CellNew clone() {
        CellNew c = new CellNew();
    ...
    public void writeCell(CellNew anotherCell) {
        CellNew cloned = anotherCell.clone();
     ...
     * @return Cell
     */
    public static CellNew fromHex(String hexBitString) {
        try {
    ...
    static List<CellNew> fromBocMultiRoot(int[] data) {
        if (data.length < 10) {
        ...
        System.out.println("cellsNum " + cellsNum + ", rootsNum " + rootsNum);
        r.readBytes(dataSizeBytes);
        ...
        List<CellNew> cells = parseCells(rootsNum, cellsNum, bocFlags.cellNumSizeBytes, payload, index);

        return cells;
    }

    private static List<CellNew> parseCells(long rootsNum, long cellsNum, int refSzBytes, int[] data, int[] index) {
        CellNew[] cells = new CellNew[(int) cellsNum];
...
//                refsIndex[x++] = Utils.concatBytes(dynInt(refsIndex), refsIndex);
            ...
            CellNew[] refs = new CellNew[refsIndex.length];
            ...
            CellNew newCell = new CellNew(new BitString(payload, bitSz), bitSz, Arrays.asList(refs), special, levelMask);
            cells[i] = newCell; // not by index i
        }

        System.out.println("cells " + cells.length);

        List<CellNew> roots = new ArrayList<>((int) rootsNum);
...
//        roots.add(cells.get(0));
     ...
     * @return TreeWalkResult, topologicalOrderArray and indexHashmap
     */
    TreeWalkResult treeWalk(CellNew cell, List<TopologicalOrderArray> topologicalOrderArray, HashMap<String, Integer> indexHashmap, String parentHash) {
    ...
    void moveToTheEnd(HashMap<String, Integer> indexHashmap, List<TopologicalOrderArray> topologicalOrderArray, String target) {
        ...
        for (CellNew subCell : data.cell.refs) {
