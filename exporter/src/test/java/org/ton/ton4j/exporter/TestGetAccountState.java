...
public class TestGetAccountState {
  ...
  public void testGetAccountStateVsGetAccountByAddress() throws IOException {
    ...
    if (accountFromState != null && accountFromAddress != null) {
      ...
      log.info("  getAccountState() balance: {}", Utils.formatNanoValue(accountFromState.getBalance()));
      log.info("  getAccountByAddress() balance: {}", Utils.formatNanoValue(accountFromAddress.getBalance()));
  ...
  public void testGetAccountStateWithLatestBlock() throws IOException {
    ...
    log.info("  Shard: {}", latestBlock.getBlockInfo().getShard().convertShardIdentToShard().toString(16));
    ...
    if (account != null) {
      log.info("Account state retrieved from recent blockchain state (seqno: {})", latestBlock.getBlockInfo().getSeqno());
      log.info("  Balance: {}", Utils.formatNanoValue(account.getBalance()));
    } else {
