...
public class TonExporterApp {
  ...
  static {
    // Configure JVM to use maximum available system memory
    configureOptimalMemoryUsage();
  }

  /**
   * Configures JVM memory settings to use 80% of available system RAM.
   * This method calculates the optimal heap size and provides recommendations.
   */
  private static void configureOptimalMemoryUsage() {
    try {
      ...
      System.err.println("=== TON4J Exporter Memory Configuration ===");
      System.err.println("Current JVM Memory Status:");
      System.err.printf("  JVM Max Heap: %.1fGB%n", maxMemory / (1024.0 * 1024.0 * 1024.0));
      System.err.printf("  JVM Total: %.1fGB%n", totalMemory / (1024.0 * 1024.0 * 1024.0));
      System.err.printf("  JVM Free: %.1fGB%n", freeMemory / (1024.0 * 1024.0 * 1024.0));
      System.err.printf("  System Total: %.1fGB%n", systemTotalMemory / (1024.0 * 1024.0 * 1024.0));
      
      // Calculate 80% of system memory
      long targetHeapSize = (long) (systemTotalMemory * 0.8);
      
      if (maxMemory < targetHeapSize) {
        long recommendedHeapGB = targetHeapSize / (1024 * 1024 * 1024);
        System.err.println();
        System.err.println("⚠️  MEMORY OPTIMIZATION RECOMMENDATION:");
        System.err.printf("   Current heap (%.1fGB) is less than optimal (%.1fGB - 80%% of system memory)%n", 
                         maxMemory / (1024.0 * 1024.0 * 1024.0), 
                         targetHeapSize / (1024.0 * 1024.0 * 1024.0));
        System.err.println();
        System.err.println("🚀 For optimal performance, restart with:");
        System.err.printf("   java -Xmx%dgG -jar TonExporterApp.jar [your arguments]%n", recommendedHeapGB);
        System.err.println();
        System.err.println("💡 Additional JVM tuning for large heaps:");
        System.err.printf("   java -Xms4g -Xmx%dgG -XX:+UseG1GC -XX:MaxGCPauseMillis=200 \\%n", recommendedHeapGB);
        System.err.println("        -XX:G1HeapRegionSize=32m -XX:+UseStringDeduplication \\");
        System.err.println("        -XX:+AlwaysPreTouch -jar TonExporterApp.jar [your arguments]");
        System.err.println();
        
        // Try to expand heap usage within current limits
        try {
          // Force garbage collection first
          System.gc();
          
          // Allocate large arrays to force JVM to use more of available heap
          long availableHeap = maxMemory - (totalMemory - freeMemory);
          if (availableHeap > 1024 * 1024 * 1024) { // If more than 1GB available
            int arraySize = (int) Math.min(Integer.MAX_VALUE - 8, availableHeap / 4 / 8); // Use 1/4 of available
            if (arraySize > 0) {
              @SuppressWarnings("unused")
              byte[] memoryExpander = new byte[arraySize];
              memoryExpander = null; // Allow GC
              System.err.println("✓ Expanded memory usage within current heap limits");
            }
          }
        } catch (OutOfMemoryError e) {
          System.err.println("⚠️  Cannot expand memory usage further within current heap limits");
        }
        
        System.err.println("Continuing with current heap size...");
        System.err.println("==========================================");
        System.err.println();
      } else {
        System.err.println("✓ JVM heap size is already optimal for system memory");
        System.err.println("==========================================");
        System.err.println();
      }
      
    } catch (Exception e) {
      System.err.println("Warning: Failed to configure optimal memory usage: " + e.getMessage());
    }
  }
  
  /**
   * Gets total system memory in bytes by reading /proc/meminfo on Linux systems.
   * Falls back to JVM max memory estimation if unable to read system memory.
   */
  private static long getSystemTotalMemory() {
    try {
      ...
      System.err.println("Debug: Could not read system memory from /proc/meminfo: " + e.getMessage());
    }
    
    // Fallback estimation: assume system has 4x the current JVM max heap
    // This is a rough estimate for cases where /proc/meminfo is not available
