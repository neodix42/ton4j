...
/**
 * BlockLocation represents the file location information for a block stored in packages. This
 * includes the package_id (which package file contains the block), offset within the package, and
 * size of the stored block data.
 *
 * <p>This is based on the original TON C++ implementation where the global index maps: file_hash →
 * (package_id, offset, size)
 */
@Builder
@Data
public class BlockLocation implements Serializable {
  /**
   * The package identifier that determines which package file contains this block. For Files
   * database packages: corresponds to the package file number (e.g., 0000000100.pack) For
   * traditional archive packages: corresponds to the archive sequence number
   */
  long packageId;
  ...
  public String toString() {
    return String.format(
        "BlockLocation{packageId=%d, offset=%s, size=%s, hash='%s'}",
