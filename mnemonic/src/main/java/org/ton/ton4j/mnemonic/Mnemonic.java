...
public class Mnemonic {
    ...
    private static final String DEFAULT_SALT = "TON default seed";

    // Number of PBKDF2 iterations used to check, if mnemonic phrase is valid
    private static final int DEFAULT_BASIC_ITERATIONS = 390;

    // Default salt used to check mnemonic phrase validity
    private static final String DEFAULT_BASIC_SALT = "TON seed version";

    // Number of PBKDF2 iterations used to check, if mnemonic phrase requires a password
    private static final int DEFAULT_PASSWORD_ITERATIONS = 1;

    // Default salt used to check, if mnemonic phrase requires a password
    private static final String DEFAULT_PASSWORD_SALT = "TON fast seed version";

    public static byte[] hmacSha512(byte[] key, byte[] input) throws NoSuchAlgorithmException, InvalidKeyException {
        String algorithm = "HmacSHA512";

        Mac ctx = Mac.getInstance(algorithm);
        ctx.init(new SecretKeySpec(key, algorithm));
        ctx.update(input);
        return ctx.doFinal();
    }

    private static byte[] pbkdf2Sha512(byte[] key, byte[] salt, int iterations) {
    ...
    public static boolean isBasicSeed(byte[] entropy) {
        byte[] seed = pbkdf2Sha512(entropy, DEFAULT_BASIC_SALT.getBytes(), DEFAULT_BASIC_ITERATIONS);
        return seed[0] == 0;
    }

    public static boolean isPasswordSeed(byte[] entropy) {
        byte[] seed = pbkdf2Sha512(entropy, DEFAULT_PASSWORD_SALT.getBytes(), DEFAULT_PASSWORD_ITERATIONS);
        return seed[0] == 1;
    }

    public static boolean isPasswordNeeded(List<String> mnemonic) throws NoSuchAlgorithmException, InvalidKeyException {
    ...
    public static byte[] toSeed(List<String> mnemonic, String password) throws NoSuchAlgorithmException, InvalidKeyException {
        byte[] seed = pbkdf2Sha512(toEntropy(mnemonic, password), DEFAULT_SALT.getBytes(), DEFAULT_ITERATIONS);
