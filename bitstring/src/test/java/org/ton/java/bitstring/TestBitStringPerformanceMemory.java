...
/**
 * Tests that compare memory consumption between BitString and RealBitString implementations.
 * These tests demonstrate the difference in memory footprint between the two approaches:
 * - BitString: Uses Deque<Boolean> (one Boolean object per bit)
 * - RealBitString: Uses byte[] (one bit per bit)
 */
@Slf4j
...
public class TestBitStringPerformanceMemory {
    ...
    public void testMemoryScalingWithSize() {
        ...
        log.info("Size (bits) | BitString (bytes) | RealBitString (bytes) | Ratio (BitString/RealBitString)");
        
        for (int size : sizes) {
            BitString bitString = new BitString(size);
            RealBitString realBitString = new RealBitString(size);
            ...
            if (size > 1000) {
                ...
                assertTrue("BitString should use significantly more memory for large sizes", 
    ...
    public void testMemoryConsumptionWithTypicalOperations() {
        ...
        RealBitString realBitString = new RealBitString(1024);
    ...
    public void testMemoryUsageWithMultipleInstances() {
        ...
        List<BitString> bitStrings = new ArrayList<>(instanceCount);
        List<RealBitString> realBitStrings = new ArrayList<>(instanceCount);
        
        // Create BitString instances
        for (int i = 0; i < instanceCount; i++) {
            BitString bs = new BitString(instanceSize);
        ...
        for (int i = 0; i < instanceCount; i++) {
            RealBitString rbs = new RealBitString(instanceSize);
            for (int j = 0; j < instanceSize; j++) {
                rbs.writeBit(j % 2 == 0);
            }
        ...
        log.info("Memory difference: {} bytes", Math.abs(bitStringTotalMemory - realBitStringTotalMemory));
        log.info("Ratio: BitString/RealBitString = {}", (double) bitStringTotalMemory / realBitStringTotalMemory);
    ...
    public void testLargeBitStringMemoryConsumption() {
        ...
        assertTrue("BitString should use more memory for large bit strings", 
                   bitStringMemory > realBitStringMemory);
    }
     ...
     */
    private void measureAndLog(String operation, BitString bitString, RealBitString realBitString) {
        long bitStringMemory = GraphLayout.parseInstance(bitString).totalSize();
        long realBitStringMemory = GraphLayout.parseInstance(realBitString).totalSize();
        double ratio = (double) bitStringMemory / realBitStringMemory;
