...
After successful handshake, client should start authentification, for lite-server any randomly generated key can be used, but for some services specific keys are needed. (example: storage-daemon or validator control).

1. Client should send TL message with scheme `tcp.authentificate nonce:bytes = tcp.Message`, where nonce is 1 - 512 random bytes. 
2. Server will respond with `tcp.authentificationNonce nonce:bytes = tcp.Message` which will contain 1 - 512 random bytes generated by the server.
