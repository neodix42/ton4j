...
public class CryptoUtils {
    ...
    /**
     * Compute shared key from Ed25519 private key and Ed25519 public key
     * This method converts Ed25519 keys to X25519 and computes the shared secret
     * Mirrors the Go implementation: SharedKey(ourKey ed25519.PrivateKey, serverKey ed25519.PublicKey)
    ...
    public static byte[] sharedKey(byte[] ourPrivateKey, byte[] serverPublicKey) {
        try {
            ...
            if (ourPrivateKey.length == 32) {
                ...
                throw new IllegalArgumentException("Private key must be 32 or 64 bytes, got " + ourPrivateKey.length);
            }
            
            // Ensure public key is 32 bytes
            if (serverPublicKey.length != 32) {
                throw new IllegalArgumentException("Public key must be 32 bytes, got " + serverPublicKey.length);
            }
     ...
     * This mirrors the Go implementation: x25519.EdPrivateKeyToX25519
     */
    ...
    /**
     * Build shared cipher for ADNL encryption/decryption
     * Mirrors the Go implementation: BuildSharedCipher(key []byte, checksum []byte)
     */
