...
public final class TweetNaclFast {
  ...
  */
   ...
   *
   * @param ed25519PrivKey The Ed25519 private key (32 bytes)
   * @return The X25519 private key (32 bytes)
   */
  public static byte[] convertEd25519PrivateToX25519(byte[] ed25519PrivKey) {
    if (ed25519PrivKey == null || ed25519PrivKey.length != 32) {
      throw new IllegalArgumentException("Invalid Ed25519 private key");
    }
    
    // Hash the private key using SHA-512
    byte[] h = new byte[64];
    crypto_hash(h, ed25519PrivKey, 0, 32);
    
    // Apply bit manipulations required for X25519 keys
    byte[] x25519PrivKey = new byte[32];
    System.arraycopy(h, 0, x25519PrivKey, 0, 32);
    
    // Clamp the private key as required for X25519
    x25519PrivKey[0] &= 248;
   ...
   *
   * @param ed25519PubKey The Ed25519 public key (32 bytes)
   * @return The X25519 public key (32 bytes)
   */
  public static byte[] convertEd25519PublicToX25519(byte[] ed25519PubKey) {
    if (ed25519PubKey == null || ed25519PubKey.length != 32) {
      throw new IllegalArgumentException("Invalid Ed25519 public key");
    }
    
    // Allocate arrays for unpacking the Edwards point
    long[][] p = new long[4][];
    for (int i = 0; i < 4; i++) {
      p[i] = new long[16];
    }
    
    // Unpack the Edwards point
    if (unpackneg(p, ed25519PubKey) != 0) {
      throw new IllegalArgumentException("Invalid Ed25519 public key");
    }
    
    // Convert from Edwards to Montgomery representation
    // The formula is x = (1+y)/(1-y) where y is from the Ed25519 key
    
    // Calculate 1+y and 1-y
    long[] one = new long[16];
    long[] onePlusY = new long[16];
    long[] oneMinusY = new long[16];
    long[] x = new long[16];
    
    one[0] = 1;
    
    // onePlusY = 1 + y
    A(onePlusY, 0, one, 0, p[1], 0);
    
    // oneMinusY = 1 - y
    Z(oneMinusY, 0, one, 0, p[1], 0);
    
    // Calculate inverse of (1-y)
    long[] invOneMinusY = new long[16];
    inv25519(invOneMinusY, 0, oneMinusY, 0);
    
    // x = (1+y)/(1-y)
    M(x, 0, onePlusY, 0, invOneMinusY, 0);
    
    // Pack the result into a byte array
    byte[] x25519PubKey = new byte[32];
    pack25519(x25519PubKey, x, 0);
