...
public class TestLibraryDeployer extends CommonTest {
    ...
    /**
     * tops up and deploys library deployer (in master workchain -1) that caries in the StateInit.data() v5r1 code
     */
    @Test
    public void testDeployLibraryDeployer() throws InterruptedException {

        Cell walletV5Code = CellBuilder.beginCell().
                fromBoc(WalletCodes.V5R1.getValue()).
                endCell();

        LibraryDeployer libraryDeployer = LibraryDeployer.builder()
                .tonlib(tonlib)
                .libraryCode(walletV5Code)
                .build();
        ...
        BigInteger balanceLib = TestFaucet.topUpContract(tonlib, Address.of(nonBounceableAddressLib), Utils.toNano(1));
        log.info("new wallet {} balance: {}", libraryDeployer.getName(), Utils.formatNanoValue(balanceLib));
        libraryDeployer.deploy();
//
        Utils.sleep(10, "deployment of LibraryDeployer will never happen an Lite-server will return an error, but the library will be deployed");

        SmcLibraryResult smcLibraryResult = tonlib.getLibraries(Collections.singletonList(
