...
public class TestTonSdkTestCasesSmartContracts {
  ...
  public void testSmartContracts1() {
    ...
    assertThat(WalletV3R2.builder().build().createCodeCell().toHex().toUpperCase())
        .isEqualTo(bocAsHex);
  }
  ...
  public void testSmartContracts2() {
    ...
    WalletV3R2 contract =
        WalletV3R2.builder().wc(workchain).keyPair(keyPair).walletId(walletId).build();

    String expectedRawAddress = (String) testCase.getExpectedOutput().get("rawAddress");
    String expectedBocAsHex =
        (String) testCase.getExpectedOutput().get("externalMessageBocAsHexWithCrc");
  ...
  public void testSmartContracts3() {
    ...
    String expectedExternalMessageAsHex =
        (String) testCase.getExpectedOutput().get("externalMessageAsHex");
    String expectedBocAsHex =
        (String) testCase.getExpectedOutput().get("externalMessageBocAsHexWithCrc");
    ...
    assertThat(contract.createCodeCell().bitStringToHex().toUpperCase())
        .isEqualTo(expectedCodeAsHex);
    assertThat(contract.createDataCell().bitStringToHex().toUpperCase())
        .isEqualTo(expectedDataAsHex);
    Message msg = contract.prepareDeployMsg();
  ...
  public void testSmartContracts8() {
    ...
    WalletV4R2 contract =
        WalletV4R2.builder().wc(workchain).keyPair(keyPair).walletId(walletId).build();
  ...
  public void testSmartContracts14() {
    ...
    BigInteger expectedBalanceOfJettonsAtRandomAddress =
        new BigInteger(
            testCase.getExpectedOutput().get("balanceOfJettonsAtRandomAddress").toString());
  ...
  public void testSmartContracts17() throws InterruptedException, NoSuchAlgorithmException {
    ...
    for (Destination destination : dummyDestinations) {
      BigInteger balanceOfDestinationWallet =
          tonlib.getAccountBalance(Address.of(destination.getAddress()));
    ...
    assertThat(WalletCodes.highloadV3.getValue())
  ...
  public void testSmartContracts18() throws InterruptedException {
    ...
    BigInteger expectedNeojAtRandomAddress =
        new BigInteger(testCase.getExpectedOutput().get("neojAtRandomAddress").toString());
  ...
  public void testSmartContracts19() throws InterruptedException, NoSuchAlgorithmException {
    ...
    BigInteger expectedTotalSumOfJettonsAt300Addresses =
        new BigInteger(
            testCase.getExpectedOutput().get("totalSumOfJettonsAt300Addresses").toString());
    ...
    JettonWallet myHighLoadJettonWallet =
        jettonMinterWallet.getJettonWallet(highloadWalletV3.getAddress());
    ...
    for (Address destination : globalDummyDestinations) {
      ...
      BigInteger jettonBalanceOfDestinationWallet =
          ContractUtils.getJettonBalance(
              tonlib, Address.of(neojMasterContractAddress), destination);
      log.info("{} : {}", destination.toBounceable(), jettonBalanceOfDestinationWallet);
      totalSum = totalSum.add(jettonBalanceOfDestinationWallet);
      log.info("totalSum {} out of {}", totalSum, expectedTotalSumOfJettonsAt300Addresses);
    }
  ...
  List<Destination> createDummyDestinations(
      int count, BigDecimal amount, Boolean bounceFlag, int sendMode)
  ...
  List<Destination> createDummyJettonDestinations(
      int count,
