...
public class TestPayments extends CommonTest {
  ...
  public void testPayments() throws InterruptedException {
    ...
    // 0.05 TON is the amount to execute this transaction on the blockchain. The unused portion will
    // be returned.
    ...
    config =
        WalletV3Config.builder()
            ...
            .amount(
                channelInitState
                    .getBalanceA()
                    .add(Utils.toNano(0.05))) // // +0.05 TON to network fees
    ...
    // A creates new state - subtracts 0.1 from A's balance, adds 0.1 to B's balance, increases A's
    // seqno by 1
    ChannelState channelState1 =
    ...
    // B checks that the state is changed according to the rules, signs this state, send signed
    // state to A (e.g. via websocket)
    ...
    // A creates new state - subtracts 0.2 from A's balance, adds 0.2 to B's balance, increases A's
    // seqno by 1
    ...
    // B creates new state - subtracts 1.1 from B's balance, adds 1.1 to A's balance, increases B's
    // seqno by 1
    ...
    // Note that a party can make its transfers (from itself to another) asynchronously without
    // waiting for the action of the other side.
    // Party must increase its seqno by 1 for each of its transfers and indicate the last seqno and
    // balance of the other party that it knows.

    // ----------------------------------------------------------------------
    // CLOSE PAYMENT CHANNEL

    // The parties decide to end the transfer session.
    // If one of the parties disagrees or is not available, then the payment channel can be
    // emergency terminated using the last signed state.
    ...
    // Payment channel smart contract will send funds to participants according to the balances of
    // the state.
  ...
  public void testPaymentsAdnlLiteClient() throws Exception {
    ...
    // The payment channel is established between two participants A and B.
    // Each has own secret key, which he does not reveal to the other.
    walletA = GenerateWallet.randomV3R1(adnlLiteClient, 6);
    walletB = GenerateWallet.randomV3R1(adnlLiteClient, 6);
    walletAddressA = walletA.getAddress();
    walletAddressB = walletB.getAddress();
    log.info("walletA address {}", walletAddressA);
    log.info("walletB address {}", walletAddressB);
