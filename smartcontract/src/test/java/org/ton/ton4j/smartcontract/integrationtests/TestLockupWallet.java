...
public class TestLockupWallet extends CommonTest {
  ...
  public void testDeployWalletLockup() throws IOException, InterruptedException {
    ...
    TweetNaclFast.Signature.KeyPair sigKeyPair =
        Utils.generateSignatureKeyPairFromSeed(boxKeyPair.getSecretKey());

    log.info("restricted-validator-wallet-001.pk {}", Utils.bytesToHex(boxKeyPair.getSecretKey()));

    // echo 'hex-prv-key' | xxd -r -p  >
    // /usr/local/bin/mytoncore/wallets/restricted-validator-wallet-001.pk
        ...
        LockupWalletV1.builder()
            ...
            .lockupConfig(
                LockupConfig.builder()
                    ...
                    .totalRestrictedValue(Utils.toNano(5_000_000))
                    .allowedDestinations(Arrays.asList(elector.toString(), myWallet.toString()))
                    .build())
            .build();
